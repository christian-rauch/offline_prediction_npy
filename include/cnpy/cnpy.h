...
namespace cnpy {
    ...
    template<typename T> void npy_save(std::string fname, const T* data, const unsigned int* shape, const unsigned int ndims, std::string mode = "w") {
        ...
        if(fp) {
            ...
            printf("cnpy error: %s has word size %d but npy_save appending data sized %d\n", fname.c_str(), word_size, sizeof(T));
		//assert( word_size == sizeof(T) );
            }
            if(tmp_dims != ndims) {
            printf("cnpy error: npy_save attempting to append misdimensioned data to %s\n", fname.c_str());
                //assert(tmp_dims == ndims);
            }

            for(unsigned int i = 1; i < ndims; i++) {
                if(shape[i] != tmp_shape[i]) {
            printf("cnpy error: npy_save attempting to append misshaped data to %s\n", fname.c_str());
	...
	if(!fp) {
      printf("npy_gzsave: Error! Unable to open file %s!\n",fname.c_str());
	}
	std::vector<char> header = create_npy_header(data,shape,ndims);
