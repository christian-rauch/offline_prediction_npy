...
void cnpy::parse_npy_header(FILE* fp, unsigned int& word_size, unsigned int*& shape, unsigned int& ndims, bool& fortran_order) {  
    ...
    if (res != 11)
        printf("cnpy::parse_npy_header read discprepancy");
...
void cnpy::parse_zip_footer(FILE* fp, unsigned short& nrecs, unsigned int& global_header_size, unsigned int& global_header_offset)
{
    ...
    if (res != 22)
       printf("cnpy::parse_zip_footer read discprepancy");
...
cnpy::NpyArray load_the_npy_file(FILE* fp) {
    ...
    if (nread != size)
        printf("cnpy::load_the_npy_file read size discrepancy");
    return arr;
}
...
cnpy::NpyArray gzload_the_npy_file(gzFile fp) {
    ...
    if (nread != size*word_size)
        printf("cnpy::gzload_the_npy_file read size discrepancy");
    return arr;
}
...
cnpy::npz_t cnpy::npz_load(std::string fname) {
    ...
    while(1) {
        ...
        if (headerres != 30)
        printf("cnpy::npz_load read discprepancy on header");
        ...
        if (vname_res != name_len) 
            printf("cnpy::npz_load read discprepancy on name_len");
        ...
        if(extra_field_len > 0) {
            ...
            if (efield_res != extra_field_len) 
                printf("cnpy::npz_load read discprepancy on extra_field_len");
        }
...
void cnpy::parse_npy_gzheader(gzFile fp, unsigned int& word_size, unsigned int*& shape, unsigned int& ndims, bool& fortran_order) {  
    ...
    if (res != 11)
        printf("cnpy::parse_npy_gzheader read discprepancy");
